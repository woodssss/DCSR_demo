...
if __name__ == "__main__":
    ...
    load_epoch_gen, load_epoch_0, load_epoch_1= 2000, 1000, 1000
    load_epoch_2 = 350
    #load_epoch_2 = None
    ...
    min, t1, t2 = find_inter_t(train, cc, marginal_prob_std_fn_b, t1_ls, cmin=4, cmax=6)
    t = 1
    ...
    eps1, eps2, eps3 = 1e-3, 1e-3, 1e-3
    method1, method2, method3 = 'pc_sampler_cond', 'pc_sampler_cond', 'pc_sampler_cond'
    ...
    # eps1, eps2, eps3 = 1e-5, 1e-5, 1e-3
    # method1, method2, method3 = 'ode_solver_cond', 'ode_solver_cond', 'pc_sampler_cond'
    ...
    ### By direct Super-resolution
    X = torch.from_numpy(cc[:bs, ..., [0]]).float().to(device)
    c_res_ls = direct_super(marginal_prob_std_fn_s, diffusion_coeff_fn_s, points_x_0, points_x_1, points_x_2, points_x, L, eps, t, X, model_up_0, model_up_1=model_up_1, model_up_2=model_up_2, method1=method1, method2=method2, method3=method3, eps1=eps1/2, eps2=eps2/2, eps3=eps3/2)
    ...
    cwd = os.getcwd()
    log_name = cwd + '/figs/ELAS_Error_res_type'+ type+ '_problem_' + str(problem) + '.txt'
    fig_name_process = cwd + '/figs/ELAS_process_type'+ type+ '_problem_' + str(problem) + '.png'
    fig_name_ins = cwd + '/figs/ELAS_ins_problem_type'+ type+ '_problem_' + str(problem) + '.png'
    fig_name_spectral = cwd + '/figs/ELAS_spectral_type'+ type+ '_problem_' + str(problem) + '.png'
    npy_name = cwd + '/figs/ELAS_type_' + type + '_problem_' + str(problem) + '.npy'
    ...
    fig2, ax = plt.subplots(bs_plt, nrows, figsize=(nrows * 2, bs_plt * 2))
    for i in range(bs_plt):
        # ax[i, 0].contourf(xx_0, yy_0, cc[i, ..., 0], 36, cmap=cm.jet)
        # ax[i, 1].contourf(xx, yy, c_256[i, ..., 0], 36, cmap=cm.jet)
        # ax[i, 2].contourf(xx, yy, c_up_256[i, ..., 0], 36, cmap=cm.jet)
        # ax[i, 3].contourf(xx, yy, pd_256[i, ..., 0], 36, cmap=cm.jet)
        # ax[i, 4].contourf(xx, yy, real_256[i, ..., 0], 36, cmap=cm.jet)
