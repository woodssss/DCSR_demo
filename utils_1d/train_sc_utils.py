...
def loss_score_t(model, x, marginal_prob_std, eps=1e-5):
  ...
  std = marginal_prob_std(random_t)
  perturbed_x = x + z * std[:, None, None]
  # print(z.shape, perturbed_x.shape)
  # zxc
...
def transfer_sdit_traj(marginal_prob_std_fn, diffusion_coeff_fn, model_down_gen, X, eps, t1, t2):
    ic = X if torch.is_tensor(X) else torch.from_numpy(X).float().to(device)
    ...
    tmp_latent_x = get_perturbed_x(X, marginal_prob_std_fn, t=t1 / 20)
    x_ls.append(tmp_latent_x.detach().cpu().numpy())

    tmp_latent_x = get_perturbed_x(X, marginal_prob_std_fn, t=t1 / 10)
    x_ls.append(tmp_latent_x.detach().cpu().numpy())

    tmp_latent_x = get_perturbed_x(X, marginal_prob_std_fn, t=t1 / 2)
    x_ls.append(tmp_latent_x.detach().cpu().numpy())

    latent = get_perturbed_x(ic, marginal_prob_std_fn, t=t1)
    x_ls.append(latent.detach().cpu().numpy())

    y_ls = []

    sample_b = ode_solver(model_down_gen, marginal_prob_std_fn, diffusion_coeff_fn, latent, forward=2, eps=t2/2, T=t2)
    y_ls.append(sample_b.detach().cpu().numpy())

    sample_b = ode_solver(model_down_gen, marginal_prob_std_fn, diffusion_coeff_fn, latent, forward=2, eps=t2 / 10, T=t2)
    y_ls.append(sample_b.detach().cpu().numpy())

    sample_b = ode_solver(model_down_gen, marginal_prob_std_fn, diffusion_coeff_fn, latent, forward=2, eps=t2 / 20, T=t2)
    y_ls.append(sample_b.detach().cpu().numpy())

    sample_down = ode_solver(model_down_gen, marginal_prob_std_fn, diffusion_coeff_fn, latent, forward=2, eps=eps, T=t2)

    y_ls.append(sample_down.detach().cpu().numpy())
