...
def compute_all_error(pd, ref, max_k):
    ...
    MMD = compute_mmd(pd, ref, sigma=1)
...
def find_inter_t(X, Y, marginal_prob_std_fn, t1_ls, cmin=4, cmax=8, metric= 'spectrum', max_k = 16, sigma=0.1):
    ...
    opt_t2 = 100
    for t1 in t1_ls:
        t2_ls = np.linspace(cmin*t1, cmax*t1, 10)
        for t2 in t2_ls:
            #print(t1, t2)
            ...
            if metric == 'spectrum':
                ...
                _, dist = compute_melr(E_x, E_y, max_k=max_k, weighted=True)
