...
if __name__ == "__main__":
    ...
    # print(l1.shape, ref.shape, sp.shape, cv1.shape, cv2.shape)
    # zxc
    ...
    ### low measurement
    cc = sp + 0.1*np.random.randn(*sp.shape)
    ...
    for i in range(1, bs):
        img_top = ax[0, i].contourf(xx_0, yy_0, cc[i * 2, ..., 0], 36, cmap=cm.jet)
        images_top.append(img_top)

        img_mid = ax[1, i].contourf(xx_0, yy_0, transfer_sdit_0[i * 2, ..., 0], 36, cmap=cm.jet)
        images_mid.append(img_mid)

        img_bottom = ax[2, i].contourf(xx_0, yy_0, target[i * 2, ..., 0], 36, cmap=cm.jet)
    ...
    ax[2, 0].text(0.5, 0.5, r'$\tilde{u}^h$', fontsize=16, ha='center', va='center')
